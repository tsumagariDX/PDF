...
def build_reorder_tab(app):
    ...
    tk.Label(
        title_frame,
        text="サムネイルをドラッグして並び替え、選択ページを回転できます",
    ...
    # 左：サムネ領域（可変幅）
    left_panel = ttk.LabelFrame(main_container, text="ページ並び替え（サムネイル）", padding=10)
    ...
    # 右：操作（固定幅）
    right_panel = ttk.LabelFrame(main_container, text="操作", padding=10)
    right_panel.pack(side="left", fill="both", expand=False, padx=(5, 0))
    right_panel.pack_propagate(False)
    right_panel.configure(width=350)

    # -------- 左パネル：サムネビュー --------
    app.reorder_thumb_view = PageThumbnailView(left_panel, thumb_height=100)
    ...
    rotate_frame = tk.Frame(right_panel, bg=Colors.BG_MAIN)
    ...
    execute_btn = ModernButton(right_panel, text="🚀 並び替え/回転を実行", command=run_reorder, style="primary")
