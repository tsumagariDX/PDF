...
def build_reorder_tab(app):
    ...
    # 右：操作（固定幅）
    right_panel = ttk.LabelFrame(main_container, text="操作", padding=5)
    right_panel.pack(side="left", fill="both", expand=False, padx=(5, 0))
    right_panel.configure(width=255)  # 幅を少し広げてスクロールバーのスペースを確保
    
    # 右パネル内にスクロール可能なフレームを作成
    ...
    right_content.configure(bg=Colors.BG_MAIN)  # 背景色を設定
