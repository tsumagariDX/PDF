...
def build_merge_tab(app):
    ...
    # スクロール可能なフレームを作成
    scroll_container = ScrollableFrame(app.tab_merge)
    ...
    # パディング用の外側フレーム
    padded_container = tk.Frame(container, bg=Colors.BG_MAIN)
    padded_container.pack(fill="both", expand=True, padx=Config.PADDING_LARGE, pady=Config.PADDING_LARGE)
    
    # 以下、containerの代わりにpadded_containerを使用
    ...
    left_panel.configure(width=294)
    ...
    move_col = tk.Frame(listbox_frame, bg=Colors.BG_MAIN,width=32)
