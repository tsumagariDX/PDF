...
def build_password_tab(app):
    ...
    def _toggle_operation_mode():
        if app.password_operation_mode.get() == "set":
            app.password_set_container.pack(fill="x", pady=(0, 10))
            app.password_remove_container.pack_forget()
        else:
            app.password_remove_container.pack(fill="x", pady=(0, 10))
    ...
    right_bottom = tk.Frame(right_panel, bg=Colors.BG_MAIN)
    right_bottom.pack(side="bottom", fill="x", pady=(10, 0))
    
    right_content = tk.Frame(right_panel, bg=Colors.BG_MAIN)
    right_content.pack(fill="both", expand=True)

    # Operation mode (set/remove)
    app.password_operation_mode = tk.StringVar(value="set")

    operation_frame = tk.Frame(right_content, bg=Colors.BG_MAIN)
    ...
    make_output_folder_row(right_bottom, app=app)
    
    output_frame = tk.Frame(right_bottom, bg=Colors.BG_MAIN)
    ...
    app.init_placeholder(app.password_filename_entry, "{name}_protected.pdf")

    # オプション
    from src.ui.tab_base import make_options_checkboxes
    make_options_checkboxes(right_bottom, app=app)
