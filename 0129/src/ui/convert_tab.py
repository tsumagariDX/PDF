...
def build_convert_tab(app):
    ...
    tk.Label(
        title_frame,
        text="（※PDFから文字認識できたものを出力する機能であり、元のレイアウトは維持できません）",
    ...
    def _maybe_update_name_placeholder():
        ...
        if current == "":
            app.set_placeholder(app.convert_name_pattern_entry, "空欄:'元ファイル名'_変換済み")
    ...
    def _on_mousewheel(event):
        app.convert_listbox.yview_scroll(int(-1 * (event.delta / 120)), "units")
    app.convert_listbox.bind("<MouseWheel>", _on_mousewheel)
    app.convert_listbox.bind("<Button-4>", lambda e: app.convert_listbox.yview_scroll(-1, "units"))
    app.convert_listbox.bind("<Button-5>", lambda e: app.convert_listbox.yview_scroll(1, "units"))
    ...
    # ===== Right panel widgets =====
    # Output type
    type_frame = ttk.LabelFrame(right_content, text="📦 出力形式", padding=8)
    ...
    app.init_placeholder(app.convert_name_pattern_entry, "空欄:'元ファイル名'_変換済み")
    ...
    note = ttk.Label(
        ...
        text=(
            ...
            "※ Excel出力では、読み取れた表をセル分割し、罫線付きで出力します。"
        ),
        wraplength=520,
        justify="left",
    )
    ...
    # Execute
    btn_exec = ModernButton(right_content, text="🚀 変換を実行", command=run_convert, style="primary")
