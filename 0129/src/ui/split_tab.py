...
def build_split_tab(app):
    ...
    # 左パネル（ScrollableFrame削除 - PageSelectView内部でスクロール）
    left_panel = ttk.LabelFrame(main_container, text="📄 ページ選択 / プレビュー", padding=5)
    main_container.add(left_panel, weight=3)

    # 右パネル（スクロール可能）
    right_panel = ttk.LabelFrame(main_container, text="⚙️ 設定", padding=5)
    ...
    # appに保存
    app.split_right_scroll = right_scroll
    ...
    app.split_thumb_view = PageSelectView(left_content, thumb_height=100)
