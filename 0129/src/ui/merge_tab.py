...
def build_merge_tab(app):
    ...
    def move_to_top():
        ...
        if not sel or sel[0] == 0:
            return
        idx = sel[0]
        item = app.pdf_paths.pop(idx)
        app.pdf_paths.insert(0, item)
        _refresh_merge_list(keep_selection=False)
        app.merge_listbox.selection_set(0)
        app.merge_listbox.see(0)
    ...
    def move_to_bottom():
        ...
        idx = sel[0]
        item = app.pdf_paths.pop(idx)
        app.pdf_paths.append(item)
        _refresh_merge_list(keep_selection=False)
        app.merge_listbox.selection_set(len(app.pdf_paths) - 1)
        app.merge_listbox.see(len(app.pdf_paths) - 1)
    ...
    # ====================
    # PanedWindowを使用してユーザーが左右のサイズを調整可能に
    ...
    main_container.add(left_panel, weight=1)  # weight=1で初期比率を設定

    # Right: settings/execute
    right_panel = ttk.LabelFrame(main_container, text="⚙️ 設定", padding=10)
    main_container.add(right_panel, weight=1)  # weight=1で初期比率を設定
    ...
    def _on_mousewheel(event):
        app.merge_listbox.yview_scroll(int(-1 * (event.delta / 120)), "units")
    app.merge_listbox.bind("<MouseWheel>", _on_mousewheel)
    app.merge_listbox.bind("<Button-4>", lambda e: app.merge_listbox.yview_scroll(-1, "units"))
    app.merge_listbox.bind("<Button-5>", lambda e: app.merge_listbox.yview_scroll(1, "units"))
    ...
    summary_frame = ttk.LabelFrame(right_content, text="📌 情報", padding=10)
    ...
    name_frame = tk.Frame(right_content, bg=Colors.BG_MAIN)
    ...
    execute_btn = ModernButton(right_content, text="🚀 PDFを結合する", command=execute_merge, style="primary")
